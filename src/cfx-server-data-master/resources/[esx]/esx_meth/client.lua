...
RegisterNetEvent('MF_DopeMeth:UseSeed')
RegisterNetEvent('MF_DopeMeth:UseItem')
RegisterNetEvent('MF_DopeMeth:UseBag')
RegisterNetEvent('MF_DopeMeth:UseRollingPapers')
...
function MFD:Start()
  ...
  ESX.TriggerServerCallback('MF_DopeMeth:GetLoginData', function(plants)  
  ...
  if self.CanHarvest or self.PolText then 
    if Utils:GetKeyPressed("E") and (GetGameTimer() - self.timer) > 200 and not self.CurInteracting then
      Citizen.CreateThread(function()
        ...
        TaskStartScenarioInPlace(plyPed, "", 0, true)
        Wait(20000)
        TaskStartScenarioInPlace(plyPed, "", 0, false)
        Wait(1000)
...
function MFD:TextHandler()
  ...
  if closest.Gender == "Male" then
    strA = "[~o~Preparation of Methamphetamine~s~] "
  else
    strA = "[~o~Preparation of Methamphetamine~s~] "
  end
  ...
  local strB = colFert .. "Ingredients~s~] " 
...
function MFD:GrowPlantSlow(plant,key)
  ...
  if plant.Stage ~= math.min(targetStage,3) then
...
function MFD:Awake(...)
    ...
    ESX.TriggerServerCallback('MF_DopeMeth:GetStartData', function(retVal) self.dS = true; self.cS = retVal; end)
...
function MFD:Sync(plant,delete)
  TriggerServerEvent('MF_DopeMeth:SyncPlant',plant,delete)
end
...
AddEventHandler('MF_DopeMeth:UseSeed', function(seed) if MFD.iR then MFD:UseSeed(seed); end; end)
AddEventHandler('MF_DopeMeth:UseItem', function(item) if MFD.iR then MFD:UseItem(item); end; end)
AddEventHandler('MF_DopeMeth:SyncPlant', function(plant,del) if MFD.iR then MFD:SyncHandler(plant,del); end; end)
AddEventHandler('MF_DopeMeth:UseBag', function(canUse,msg) MFD:UseBag(canUse,msg); end)
